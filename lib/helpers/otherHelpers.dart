...
class Helper {
  ...
  //check internet connectivity - Web compatible
  Future<bool> checkConnectivity() async {
    // For web platform, return true by default or implement web check
    if (kIsWeb) {
      try {
 ...
 Future<String> barcodeScan(BuildContext context) async {
  ...
  //request permissions - Web compatible
  ...
  //job scheduler - Web compatible (skip on web)
  jobScheduler() {
    if (kIsWeb) {
      return; // Cron jobs not supported on web
    }
  ...
  //post call_logs in api - Web compatible
  syncCallLogs() async {
    // Skip on web
    if (kIsWeb) {
      return;
    ...
    if (await Permission.phone.status == PermissionStatus.granted) {
      if (Config().syncCallLog && await Helper().checkConnectivity()) {
        ...
        try {
          // Note: CallLog functionality is commented out as it requires native integration
          // Add your call log sync logic here if needed
        } catch (e) {}
      }
    }
  }
  ...
  savePdf(sellId, taxId, context, invoiceNo, {invoice}) async {
    ...
    final pdfDocument = await Printing.convertHtml(
      format: const pd.PdfPageFormat(5595.44, 841),
      html: _invoice,
    );
  ...
  // ========== EFRIS HELPER METHODS ==========

  // Check if EFRIS is enabled for the business
  Future<bool> isEfrisEnabled() async {
    try {
      List business = await System().get('business');
      if (business.isNotEmpty && business[0].containsKey('efris_enabled')) {
        return business[0]['efris_enabled'] == 1 ||
            business[0]['efris_enabled'] == true;
      }
      return false;
    } catch (e) {
      return false;
  ...
  // Get EFRIS TIN
  Future<String?> getEfrisTin() async {
    try {
      List business = await System().get('business');
      if (business.isNotEmpty && business[0].containsKey('tin')) {
        return business[0]['tin'];
      }
      return null;
    } catch (e) {
      return null;
  ...
  // Format invoice for EFRIS
  Map<String, dynamic> formatInvoiceForEfris(Map<String, dynamic> invoiceData) {
    return {
      'invoice_no': invoiceData['invoice_no'],
      'tin': invoiceData['tin'],
      'device_no': invoiceData['device_no'] ?? '',
      'branch_code': invoiceData['branch_code'] ?? '',
